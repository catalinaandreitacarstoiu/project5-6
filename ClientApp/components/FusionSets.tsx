...
export async function get_fusionproduct(theme:string): Promise<Models.Lego[]> {
    let res = await fetch(`./custom/FusionSets/${theme}`, { method: 'get', credentials: 'include', headers: { 'content-type': 'application/json' } })
    let json = await res.json()
    console.log("received correct products", json)
    return json
}

export class FusionSets extends React.Component<RouteComponentProps<{}>, FusionSetsComponentState> {
    ...
    componentWillMount() {
        get_fusionproduct("Fusion").then(products => this.setState({ ...this.state, products: products }))
