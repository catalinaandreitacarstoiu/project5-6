...
export async function get_ninjagoproduct(theme:string): Promise<Models.Lego[]> {
    let res = await fetch(`./custom/NinjagoSets/${theme}`, { method: 'get', credentials: 'include', headers: { 'content-type': 'application/json' } })
...
export class NinjagoSets extends React.Component<RouteComponentProps<{}>, NinjagoComponentState> {
    ...
    componentWillMount() {
        get_ninjagoproduct("Ninjago").then(products => this.setState({ ...this.state, products: products }))
